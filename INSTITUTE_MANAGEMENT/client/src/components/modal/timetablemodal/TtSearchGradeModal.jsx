...
export default function TtSearchGradeModal(props) {
    ...
    useEffect(() => {
        axios.get("http://localhost:5000/api/classes/grcs/" + props.classes).then(response => {
            console.log(response.data);
            if (response.data.length != 0) {
                setClasses(response.data);
            }
            else{
                swal({ text: "No classes for this grade", button: "Okay!"}).then((value) => {
                    window.location = ''; 
                 });
            }
        })
